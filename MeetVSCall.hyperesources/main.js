window.mainFunc = function(hypeDocument){
	...
	var waffleMenuBallButtons = ['btn_chat', 'btn_call', 'btn_meet', 'btn_share', 'btn_sch', 'btn_wb', 'btn_files', 'btn_tasks'];
	...
	function showWaffleMenu(noTrans, noActivitySelected){
		...
		var top = 90
		var col = 3
		var pw = $('#waffleMenu .btns').width()
		var left = Math.round((pw - col*ballw - (col-1)*gapx)/2)
		...
		for(var i=0; i<len; i++){
			...
			if(waffleMenuBallButtonVisible[b]){
				var l = left + cnt%col*(ballw+gapx)
				var t = top + Math.floor(cnt/col)*(ballh+gapy)
		...
		$('.activityNameBallButton').addClass('graybg');
		$('.activityNameBallButton').removeClass('whitebg');
	...
	function dismissWaffleMenu(){
		...
		$('.activityNameBallButton').addClass('whitebg');
		$('.activityNameBallButton').removeClass('graybg');
	...
	function setBubbleHeads(){
		...
		for(var i=0; i<len; i++){
			...
			if(itm.id == selectedRoomListId){
				...
				html += '<div class="ctrl" style="opacity:0; position: absolute; width:'+(headw+2)+'px; height:'+(headh+2)+'px; left:-1px; bottom:-1px; border-radius:'+(headh+2)/2+'px; background:rgba(0,0,0,0.8) url(' + hypeDocument.resourcesFolderURL() + '/ico_popout.svg) no-repeat center;"></div>';
				html += '</div>';
				html += '</div>';
				...
				for(var j=0; j<nump; j++){
					...
					if(pdat){
						...
						html += border;
						html += '<div class="avatarImage" style="width:100%; height:100%; border-radius:'+(headh/2)+'px; overflow:hidden; background-image:url(' + hypeDocument.resourcesFolderURL() + '/' + img + '); ">';
						html += '</div>';
						html += '</div>';
					}
				...
				html += '<div class="bubbleHead itmBall rosterball" style="display: inline-block; width:'+headh+'px; height:'+headh+'px; pointer-events:auto;" >';
				// ico_roster
				html += '<div class="ico_roster child" tooltip="Manage participants" title="Manage participants" style="position:absolute; transition: all 0.3s ease-out; width:'+headh+'px; height:'+headh+'px; border-radius:'+(headh/2)+'px; overflow:hidden; background-color:#333; ">';
				html += '<div class="lb" style="display:inline-block; vertical-align:middle; text-align:center; color:#fff; margin-right:2px; font-size:14px;">'+(nump+1)+'</div>';
				...
				// ico_backtomeeting
				html += '<div class="ico_backtomeeting child" tooltip="Back to meeting" title="Back to meeting" style="position:absolute; transition: all 0.3s ease-out; width:'+headh+'px; height:'+headh+'px; border-radius:'+(headh/2)+'px; overflow:hidden; background:#333 url(' + hypeDocument.resourcesFolderURL() + '/' + 'ico_backtomeeting.svg) no-repeat center;">';
				html += '</div>';
				...
				if(panel == 'incall'){
					$('.bubbleHead .ico_backtomeeting').hide();
					$('.bubbleHead .ico_backtomeeting').css({ "transform": "scale(0)" });
				}else{
					$('.bubbleHead .ico_roster').hide();
					$('.bubbleHead .ico_roster').css({ "transform": "scale(0)" });
				}			
	...
	function changeBubbleHeadsStatusTo(status){ 
		...
		if($('#room-'+selectedRoomListId+' .in_call_time').is(':visible') && (panel == 'call' || incall_room_id == selectedRoomListId) ){
			...
			$('#activity_bubble_heads').css('top', 60);
			$('#right-panel-title').css('height', inMeetingHeaderH);
			$('#right-panel-title').css('top', -5);

			if(panel != 'incall' && panel != 'call'){
				//$('#right-panel-title').css('box-shadow', '0 1px 0 rgba(0, 0, 0, 0.08)');
			}else{
				//$('#right-panel-title').css('box-shadow', '0 1px 0 rgba(0, 0, 0, 0.0)');
			}
			...
			$('#activity_bubble_heads').hide();
			$('#activity_bubble_heads').css('top', 70);
			$('#right-panel-title').css('height', 72);
			$('#right-panel-title').css('top', 0);
		...
		if(status == 'call'){
			setTimeout(function(){
					$('.bubbleHead .ico_backtomeeting').css({ "transform-origin": "100% 50%", "transform": "scale(0)" });
					setTimeout(function(){
						$('.bubbleHead .ico_roster').show();
						$('.bubbleHead .ico_roster').css({ "transform-origin": "0% 50%", "transform": "scale(1)" });
					}, 100)
				}, 300)
			...
			setTimeout(function(){
					$('.bubbleHead .ico_roster').css({ "transform-origin": "0% 50%", "transform": "scale(0)" });
					setTimeout(function(){
						$('.bubbleHead .ico_backtomeeting').show();
						$('.bubbleHead .ico_backtomeeting').css({ "transform-origin": "100% 50%", "transform": "scale(1)" });
					}, 100)
				}, 300)
	...
	function joinMeetingWithVideo(showCallmePopup){
		...
		if(people_call_or_meet == 'meet' || roomData.type == 'space' || ( (window.meet_vs_call_option == 3 || window.meet_vs_call_option == 4) && roomData.external!=true ) ){
			...
			if(people_call_or_meet == 'meet'){
				...
				setTimeout(othersJoinMeeting, 8000+2000, 'video')
			}else{
	...
	function joinMeetingWithAudio(showCallmePopup){
		...
		if(people_call_or_meet == 'meet' || roomData.type == 'space' || ( (window.meet_vs_call_option == 3 || window.meet_vs_call_option == 4) && roomData.external!=true ) ){
			...
			if(people_call_or_meet == 'meet'){
				$('#inCallPanel .waiting').show();
				setTimeout(othersJoinMeeting, 8000+2000, 'audio')
			}else{
	...
	function hideHud(){
		...
		if( $('#inCallPanel .pop_more').hitTest(mouseX, mouseY) || $('#inCallPanel .pop_share').hitTest(mouseX, mouseY) || $('#waffleMenu').is(':visible') ){
			return;
	    }
		...
		if($('#activity_bubble_heads').css('top') == '60px'){
			$('#activity_bubble_heads').animate({
	    		top: 10
		    }, {
		    	duration: 300,
		    	queue: false,
		    	complete: function(){
					...
					//$('.bubbleHead.rosterball').addClass('fadeOut');
					//setTimeout(function(){
					//	$('.bubbleHead.rosterball').hide();
					//}, 300);
	...
	function showHud(){
		...
		if($('#activity_bubble_heads').css('top') == '10px'){
			$('#activity_bubble_heads').animate({
				top: 60
	    	}, {
	    		duration: 300,
	    		queue: false,
	    		complete: function(){
	    			//$('.bubbleHead.rosterball').show();
					//setTimeout(function(){
					//	$('.bubbleHead.rosterball').removeClass('fadeOut');
					//}, 10);
	...
	function cancelSearch(){
		...
		$('#searchBox input').val('');
		$('#searchBox .lb_filter').hide();
		$('#searchBox .ico_search').css('left', '111px');
